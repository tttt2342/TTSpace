...
```mermaid
flowchart TD
    A[Set the initial conditions: \( \boldsymbol{r}_i(t_0), \boldsymbol{v}_i(t_0), \dots \)] --> B[Update neighbour list]
    B --> C[Get forces \( \boldsymbol{F}_i(t) \)]
    C --> D[Solve equations of motion over \( \delta t \)]
    D --> E[Perform \( p, T \) control (ensembles)]
    E --> F[\( t \to t + \delta t \)]
    F --> G[Calculate the desired physical quantities]
    G --> H{\( t = t_{\text{max}} ? \)}
