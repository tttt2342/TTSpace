...
Assume $$\ksi_1, \ksi_2, ..., \ksi_n$$ is a random i.i.d. variable series, with math expectation $$E(\ksi_i) = \mu$$. Then for arbitrary $$\epsilon > 0$$, there is:
\begin{equation}
\lim_{n \to \infty} p \lbrace |\frac{1}{n}\sum_{i=1}^{n} \ksi_i-\mu| < \epsilon \rbrace = 1.
\end{equation}
This means when $$n$$ goes large, the average of samples convert to math expectation.

### 2. Central Limit Theorem (CLT)
In addition to above, if the square difference $$D(\ksi_i) = \sigma^2$$, then for arbitrary real number $$\lambda$$:
\begin{equation}
\lim_{n \to \infty} p \lbrace \frac{\frac{1}{n}\sum_{i=1}^n \ksi_i-\mu}{\sigma / \sqrt{n}} < \lambda \rbrace = \frac{1}{\sqrt{2\pi}}\int_{-\infty}^\lambda e^{-(x^2 /2)} dx,
\end{equation}
